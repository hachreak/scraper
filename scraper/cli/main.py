...
def instagram_hydrate(input_, reload_every):
    with drv.load(reload_every=reload_every) as loader:
        for value in input_:
            try:
                result = iscraper.get_comments(value.strip(), loader=loader)
                if result:
    ...
    for line in input_:
        try:
            ...
            if post['id'] not in ids:
                ...
                if not language or \
                        is_lang(
                            s.remove_punctuations(post['text']).strip()
                        ) or \
                        not post['text']:
                    count_posts += 1
                    count_posts_likes += post['likes']
                    count_comments += post['comments']['count']
                    users_posting[post['username']] += 1
                    users[post['username']] += 1
                    for c in post['comments']['conversations']:
                    ...
                    for ht in post['hashtags']:
                        hashtags[ht] += 1
                    if (post.get('location', {}) or {}).get('slug'):
